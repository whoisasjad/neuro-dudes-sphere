...
export const Sidebar = () => {
  const categories = [
    { name: "AI/ML", count: 12, color: "bg-cyan-400" },
    { name: "Neuroscience", count: 8, color: "bg-purple-400" },
    { name: "Programming", count: 15, color: "bg-blue-400" },
    { name: "Quantum", count: 5, color: "bg-green-400" },
    { name: "Research", count: 7, color: "bg-pink-400" }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/40 backdrop-blur-sm rounded-lg p-6 border border-slate-700/30">
        <h3 className="text-lg font-bold text-white mb-4 flex items-center">
          <div className="w-1 h-5 bg-cyan-400 mr-3"></div>
          Categories
        </h3>
      ...
      <div className="bg-slate-800/40 backdrop-blur-sm rounded-lg p-6 border border-slate-700/30">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag}
              className="bg-slate-700/50 text-cyan-300 px-3 py-1 rounded-full text-sm hover:bg-slate-600/50 hover:text-cyan-200 transition-colors duration-200 cursor-pointer"
            >
              #{tag}
            </span>
          ))}
